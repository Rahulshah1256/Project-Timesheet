...
const authorize = require("../middleware/authorize");
    ...
    try{
            ...
            connection.query(query, (err, results) => {
              if (!err) {
                ...
                if (results.length <= 0) {
                  query = `insert into employee(name, contactNumber, email, password, iv, status, role) values
                  ('${employee.name}', '${employee.contactNumber}', '${employee.email}', '${employee.password.content}', '${employee.password.iv}', 'false', 'user')`;
                  connection.query(query, (err, results) => {
                      if (!err) {
 ...
 exports.login = async (req, res) => {
    try{
      ...
      let query = `select email, password, iv, role, status from employee where email='${employee.email}'`;
      connection.query(query, (err, results) => {
        if (!err) {
          const decryptPass = {
            iv: results[0].iv,
            content: results[0].password
          }
          const decryptPassword = authorize.decrypt(decryptPass);
          console.log(results.length, decryptPassword , employee.password);

          if (results.length <= 0 || decryptPassword != employee.password) {
